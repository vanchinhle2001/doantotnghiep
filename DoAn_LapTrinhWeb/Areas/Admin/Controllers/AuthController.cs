...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class AuthController : BaseController
    {
        ...
        public ActionResult Contact(string search, int? size, int? page)
        {
            var pageSize = (size ?? 15);
            var pageNumber = (page ?? 1);
            ViewBag.search = search;
            ViewBag.countTrash = db.Contacts.Count();
            var list = from a in db.Contacts
                       orderby a.create_at descending
                       select a;
            if (!string.IsNullOrEmpty(search))
            {
                list = from a in db.Contacts
                       where a.email.Contains(search) || a.contact_id.ToString().Contains(search) || a.name.Contains(search)
                       orderby a.create_at descending
                       select a;
            }
            return View(list.ToPagedList(pageNumber, pageSize));
        ...
        {
            Contact contact = db.Contacts.FirstOrDefault(m => m.contact_id == id);
            if(contact==null)
            {
                Notification.setNotification1_5s("Không tồn tại! (ID = " + id + ")", "warning");
