...
$('#create__save').click(function () {
    const name = $('#create__input-name').val(); 
    ...
    if (price == "") {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ discountName: name, discountPrice: price, discountStart: start, discountEnd: end, discountCode: code, quantity: quan }),
            dataType: "json",
...
//2. Chỉnh sửa
var regex = /(\d)(?=(\d{3})+(?!\d))/g;
var editOpen = function (name,id, discountprice, code, start, end, quan) {

    editModal.find('#edit__input-name').val(name);
...
$('#edit__save').click(function () {
    const name = $('#edit__input-name').val();
    ...
    if (price == "") {
        ...
        $.ajax({
            ...
            url: '/Discounts/Edit',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ discountName: name, id: discoundID, discountPrice: price, discountStart: start, discountEnd: end, discountCode: code, quantity: quan }),
            dataType: "json",
            success: function (result) {
                if (result == "exist") {
                    const Toast = Swal.mixin({
                    ...
                    $('#dis-name_' + discountName).text(name);
